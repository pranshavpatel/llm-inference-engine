...
- `phase4-hf-smoke.json`, `phase4-nanoserve-smoke.json`, and `phase4-vllm-smoke.json` are complete per-request functional replay records for that trace. All three completed 2/2 requests with the same outputs (` four,` and ` Paris.`), finish reasons (`length`), and exact token usage.
