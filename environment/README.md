...
- `phase5-vm-pilot/` preserves the L40S fixed-output trace, full HF/nanoserve/vLLM replays, launch logs, environment notes, and regenerable analysis exports. All three produced eight tokens for both requests; HF and nanoserve match exactly, while vLLM's longer output diverges on one prompt. See its `README.md` for the numerical follow-up.
